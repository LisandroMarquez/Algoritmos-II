...
int main(int argc, char *argv[]) {
    char *filepath = NULL;
    int min;
    int maxTemp[YEARS], maxPrep[YEARS];
    ...
    min = minTemperature(table);
    maxTempYearArray(table, maxTemp);

    maxPrepMonthArray(table, maxPrep);

    printf("Minimum temperature registered: %d\n", min);

    array_dump(maxTemp, 37);

    array_dump(maxPrep, 37);
